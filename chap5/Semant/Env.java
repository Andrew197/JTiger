...
class Env {
	...
	Env(ErrorMsg.ErrorMsg err) {
		...
		venv.put(sym("substring"),
				FunEntry(RECORD(sym("s"), STRING, RECORD(sym("first"), INT, RECORD(sym("n"), INT))), STRING));
		venv.put(sym("concat"), FunEntry(RECORD(sym("s1"), STRING, RECORD(sym("s2"), STRING)), STRING));
